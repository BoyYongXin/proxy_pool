...
IP_PORTS   = tools.get_conf_value(config, 'redis', 'ip_ports').split(',')
DB        = int(tools.get_conf_value(config, 'redis', 'db'))
USER_PASS = tools.get_conf_value(config, 'redis', 'user_pass')
...
class RedisDB():
    ...
    def sget(self, table, count = 0, is_pop = False):
        datas = []
        if is_pop:
    ...
    def count(self,table):
        try:
            ...
            log.error(e)
    def delete_value(self,table,value):
        try:
            return self._redis.srem(table,value)
        except Exception as e:
            log.error(e)
    def get_all(self,table):
        try:
            return list(self._redis.smembers(table))
        except Exception as e:
            log.error(e)
if __name__ == '__main__':
    redis = RedisDB()
    result = redis.get_all('proxy_pools:proxy1')
    print(len(result))
    # print(result[0:2])
